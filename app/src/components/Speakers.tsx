  ...
  return (
    <section id="speakers" className="py-24 px-6 bg-black text-white">
      <div className="container max-w-7xl mx-auto">
        <h2 className="text-4xl md:text-5xl font-black mb-16 tracking-tighter text-center"><span className="text-tedx-red">TEDx</span> SPEAKERS</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {speakersData.map((speaker) => (
            <Link key={speaker.id} to={`/speakers/${speaker.id}`} className="group relative aspect-[3/4] bg-gray-900 rounded-xl overflow-hidden border border-white/10 cursor-pointer hover:border-tedx-red/50 transition-colors block">
              <img 
                src={speaker.image} 
                alt={speaker.name} 
              ...
              <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black via-black/90 to-transparent">
                ...
                <p className="text-tedx-red font-medium">{speaker.topic}</p>
              </div>
            </Link>
          ))}
        </div>
